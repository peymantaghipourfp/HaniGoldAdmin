...
class SocketService extends GetxService {
  ...
  Future<void> connect(String url, {String? clientId}) async {
    if (_connectionStatus.value == 'connecting') {
      AppLogger.d('WebSocket: Already connecting, skipping...');
      return;
    }

    if (_isManualDisconnect) {
      AppLogger.w('WebSocket: Manual disconnect in progress, skipping connection');
      return;
    }
  ...
  void _reconnect() {
    if (_isManualDisconnect) {
      AppLogger.w('WebSocket: Manual disconnect in progress, skipping reconnection');
      return;
    }

    if (_lastUrl == null || _reconnectAttempts >= _maxReconnectAttempts) {
      AppLogger.e('WebSocket: Max reconnection attempts reached or no URL available');
      _connectionStatus.value = 'disconnected';
      return;
    }
  ...
  void _sendUserIdentification(String clientId) {
    try {
      ...
      AppLogger.d(
        'WebSocket: Sent client identification for clientId: $clientId',
      );
    } catch (e , s) {
  ...
  Future<void> disconnect() async {
    ...
    try {
      ...
      AppLogger.e('WebSocket: Error during disconnect', e, s);
    }
  ...
  Future<void> ensureConnected({String? clientId}) async {
    ...
    if (_connectionStatus.value == 'connecting') {
      AppLogger.d('WebSocket: Already connecting, waiting...');
    ...
    if (_lastUrl != null) {
      AppLogger.i('WebSocket: Ensuring connection to $_lastUrl');
      await connect(_lastUrl!, clientId: clientId);
    } else {
      AppLogger.w('WebSocket: No URL available, using default WebSocket URL');
  ...
  Future<void> initializeForNewTab() async {
    try {
      ...
      AppLogger.d(
        'WebSocket: initializeForNewTab - userId: $userId, token: ${token != null ? 'present' : 'missing'}',
      );

      if (userId != null && token != null) {
        AppLogger.i('WebSocket: Initializing for new tab with userId: $userId');
        ...
        if (_connectionStatus.value == 'connected') {
          AppLogger.d('WebSocket: Sending user identification for new tab');
          _sendUserIdentification(userId.toString());
        } else {
          AppLogger.w(
            'WebSocket: Connection not established for new tab, status: ${_connectionStatus.value}',
          );
        }
      } else {
        AppLogger.w('WebSocket: No user data found for new tab initialization');
      }
    } catch (e , s) {
      AppLogger.e('WebSocket: Error initializing for new tab', e, s);
    }
  }
