// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'socket_chat_close_broadcast.model.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

SocketChatCloseBroadcastModel _$SocketChatCloseBroadcastModelFromJson(
        Map<String, dynamic> json) =>
    SocketChatCloseBroadcastModel(
      channel: json['channel'] as String?,
      data: json['data'] == null
          ? null
          : Data.fromJson(json['data'] as Map<String, dynamic>),
    );

Map<String, dynamic> _$SocketChatCloseBroadcastModelToJson(
        SocketChatCloseBroadcastModel instance) =>
    <String, dynamic>{
      'channel': instance.channel,
      'data': instance.data,
    };

Data _$DataFromJson(Map<String, dynamic> json) => Data(
      chatId: json['chatId'] as String?,
      customerAccountId: (json['customerAccountId'] as num?)?.toInt(),
      topicId: (json['topicId'] as num?)?.toInt(),
      topicCode: json['topicCode'] as String?,
      topicKey: json['topicKey'] as String?,
      closedOn: json['closedOn'] == null
          ? null
          : DateTime.parse(json['closedOn'] as String),
      closeStatus: (json['closeStatus'] as num?)?.toInt(),
    );

Map<String, dynamic> _$DataToJson(Data instance) => <String, dynamic>{
      'chatId': instance.chatId,
      'customerAccountId': instance.customerAccountId,
      'topicId': instance.topicId,
      'topicCode': instance.topicCode,
      'topicKey': instance.topicKey,
      'closedOn': instance.closedOn?.toIso8601String(),
      'closeStatus': instance.closeStatus,
    };
